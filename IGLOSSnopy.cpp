...
const int MNM=5000; // maximal number of motifs
...
float meanfunc(float *sample, int length) {
	float x=0; 
	for(int i=0; i<length; i++) x=x+sample[i]/((float)length); 
	return x; 
	}
	
float stdev(float *sample, int length) {
	float x=0; 
	float y=meanfunc(sample,length); 
	for(int i=0; i<length; i++) x=x+(sample[i]-y)*(sample[i]-y)/((float)length); 
	return sqrt(x); 
	}
	...
	if (argc==5) {
		...
		if (file==0) printf("Can't open file %s!\n",argv[1]);
			...
			if (countnames>0) { // fasta format
				...
				char c; // character used for reading files
				int i=0;
				int j=0;
				int indx;   
				while ( (c=getc(file))!=EOF ) { 
		...
		if (ok==1) { // if all sequences of the query are of equal length and the query is properly formated
			...
			for (int iter=0; iter<noi; iter++) { // iteration begins here  
				...
				for(int i=0; i<nurows; i++) { // compute list ls for logistic fit
				...
				// logistic fit 
				mean=meanfunc(ls,nurows); 
				scale2=sqrt(3.0)*stdev(ls,nurows) / M_PI;
